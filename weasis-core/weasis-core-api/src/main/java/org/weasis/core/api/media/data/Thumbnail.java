...
public class Thumbnail<E> extends JLabel implements DragGestureListener, DragSourceListener, DragSourceMotionListener,
    ...
    public void paintSeriesState(Graphics2D g2d, int x, int y, int width, int height) {
        ...
        if (splitNb != null) {
            g2d.drawString("#" + splitNb + " [" + series.getMedias().size() + "]", x + 2, y + height - 2); //$NON-NLS-1$ $NON-NLS-2$ $NON-NLS-3$
        } else {
            g2d.drawString("[" + series.getMedias().size() + "]", x + 2, y + height - 2); //$NON-NLS-1$ $NON-NLS-2$
        }
