...
public class MinimumEnclosingRectangle {
    ...
    private int findMaxPerpendicularDistanceFromAB(List<Point2D.Double> pts, Point2D.Double A, Point2D.Double B,
        int startIndex) {
        double maxDist = distanceFromAB(pts.get(startIndex), A, B);
        ...
        while (nextDist >= maxDist) {
            ...
            nextDist = distanceFromAB(pts.get(nextIndex), A, B);
        }

        if (maxDist < minDist) {
            minDist = maxDist;
            minDistPt = pts.get(maxIndex);
            minDistSeg = new Line2D.Double(A, B);
        }
        return maxIndex;
    }
    ...
    public List<Point2D.Double> getMinimumRectangle() {
        ...
        List<Point2D.Double> rect = new ArrayList<Point2D.Double>(4);
