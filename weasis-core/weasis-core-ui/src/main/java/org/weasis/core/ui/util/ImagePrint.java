        ...
        if (pj.printDialog(aset)) {
            try {
                ...
                if (e.getMessage().indexOf("accepting job") != -1) { //$NON-NLS-1$
                    ...
                    int response =
                        JOptionPane.showConfirmDialog(null, Messages.getString("ImagePrint.print_anyway"), //$NON-NLS-1$
                            Messages.getString("ImagePrint.dialog_title"), JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE); //$NON-NLS-1$
