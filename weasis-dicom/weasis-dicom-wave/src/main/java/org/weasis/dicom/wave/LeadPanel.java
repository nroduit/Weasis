...
public class LeadPanel extends JPanel {
    ...
    private final Font fontTitle = new Font("SanSerif", Font.BOLD, 11); //$NON-NLS-1$
    ...
    private void addListeners() {
        MouseAdapter markerAdapter = new MouseAdapter() {
            @Override
        ...
        MouseAdapter basicMouseListener = new MouseAdapter() {
            @Override
            public void mouseEntered(MouseEvent e) {
                setCursor(DefaultView2d.CROSS_CURSOR);
                view.getInfoPanel().setLead(channels.getTitle());
                view.getInfoPanel().setMinMax((channels.getMinValue() + channels.getBaseline()) / 1000,
    ...
    private void updateSelection() {
        ...
        double amplitudeuV = (max - min) * channels.getAmplitudeUnitScalingFactor() + channels.getBaseline();
    ...
    private void drawGrid(Graphics2D g2) {
        ...
        double pixelPerMm = Toolkit.getDefaultToolkit().getScreenResolution() / 25.4 * view.getZoomRatio();
