...
public class RetrieveTask extends ExplorerTask<ExplorerTask<Boolean, String>, String> {
            ...
            if (callingNode == null) {
                ...
                if (RetrieveType.CGET == type) {
                    ...
                    List<AbstractDicomNode> webNodes = AbstractDicomNode.loadDicomNodes(AbstractDicomNode.Type.WEB,
                        AbstractDicomNode.UsageType.RETRIEVE, WebType.WADO);
    ...
    protected void done() {
     ...
     static DicomWebNode getWadoUrl(DicomQrView dicomQrView, String host, List<AbstractDicomNode> webNodes,
        String message1){
        ...
        if (wadoURLs.isEmpty()) {
            GuiExecutor.instance()
                .execute(() -> JOptionPane.showMessageDialog(dicomQrView.getBasePanel(),
                    message1, null, JOptionPane.ERROR_MESSAGE));
            return null;
        }
            ...
            GuiExecutor.instance().invokeAndWait(() -> {
                ...
                Object response = JOptionPane.showInputDialog(dicomQrView.getBasePanel(),
                    Messages.getString("RetrieveTask.several_wado_urls"), wadoURLs.get(0).getWebType().toString(),
    ...
    private void fillSeries() {
        ...
        boolean startDownloading = BundleTools.SYSTEM_PREFERENCES
            ...
            try {
                ...
                if (!series.isEmpty()) {
                    for (Attributes seriesDataset : series) {
                        Series<?> dicomSeries = getSeries(study, seriesDataset,loadMap, wadoParameters, baseUrl, startDownloading);
        ...
        for (MediaSeriesGroup pt : dicomModel.getChildren(MediaSeriesGroupNode.rootNode)) {
            for (MediaSeriesGroup st : dicomModel.getChildren(pt)) {
                if (st.matchIdValue(studyUID)) {
                    MediaSeriesGroup patient = explorerDcmModel.getHierarchyNode(MediaSeriesGroupNode.rootNode, pt.getTagValue(pt.getTagID()));
                    if (patient == null) {
                        MediaSeriesGroupNode p = new MediaSeriesGroupNode(pt.getTagID(), pt.getTagValue(pt.getTagID()), DicomModel.patient.getTagView());
                    ...
                    if (study == null) {
                        MediaSeriesGroupNode s = new MediaSeriesGroupNode(st.getTagID(), st.getTagValue(st.getTagID()), DicomModel.study.getTagView());
    ...
    private Series getSeries(MediaSeriesGroup study, final Attributes seriesDataset,Map<String, LoadSeries> loadMap ,WadoParameters wadoParameters ,
        String baseUrl, boolean startDownloading) {
        ...
        if (dicomSeries == null) {
            ...
            final LoadSeries loadSeries = new LoadSeries(dicomSeries, explorerDcmModel,
                BundleTools.SYSTEM_PREFERENCES.getIntProperty(LoadSeries.CONCURRENT_DOWNLOADS_IN_SERIES, 4), true,
                startDownloading);
            loadSeries.setPriority(
                new DownloadPriority(explorerDcmModel.getParent(study, DicomModel.patient), study, dicomSeries, true));
            loadMap.put(TagD.getTagValue(dicomSeries, Tag.SeriesInstanceUID, String.class), loadSeries);
        }
        return dicomSeries;
    }

    private void fillInstance(Attributes seriesDataset, Series<?> dicomSeries, Properties props, URLParameters urlParameters) {
