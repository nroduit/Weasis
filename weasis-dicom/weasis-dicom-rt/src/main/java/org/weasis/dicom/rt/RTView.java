...
import org.weasis.dicom.rt.KOComponentFactory.KOViewButton;
import org.weasis.dicom.rt.KOComponentFactory.KOViewButton.eState;
import org.weasis.dicom.rt.EventManager;
import org.weasis.dicom.rt.PRManager;
import org.weasis.dicom.rt.InfoLayer;
import org.weasis.dicom.rt.mpr.MprView.SliceOrientation;

public class RTView extends DefaultView2d<DicomImageElement> {

    private static final long serialVersionUID = 8334123827855840782L;

    private static final Logger LOGGER = LoggerFactory.getLogger(RTView.class);

    public static final ImageIcon KO_ICON = new ImageIcon(RTView.class.getResource("/icon/22x22/dcm-KO.png")); //$NON-NLS-1$
    public static final ImageIcon PR_ICON = new ImageIcon(RTView.class.getResource("/icon/22x22/dcm-PR.png")); //$NON-NLS-1$
    ...
    private Series<?> rtSeries;

    public RTView(ImageViewerEventManager<DicomImageElement> eventManager) {
        super(eventManager);
                        ...
                        if (sliceGeometry != null && fruid != null) {
                            ...
                            if (container != null) {
                                ...
                                for (DefaultView2d<DicomImageElement> v : viewpanels) {
                                    ...
                                    if (v instanceof RTView && fruid.equals(s.getTagValue(TagW.FrameOfReferenceUID))) {
                                        ((RTView) v).computeCrosshair(p3);
                                        v.repaint();
                                    }
                                }
                            }
    ...
    public void updateKOButtonVisibleState() {
        ...
        for (ViewButton vb : getViewButtons()) {
            if (vb != null && vb.getIcon() == RTView.KO_ICON) {
...
//            series.setSelected(true, null);
    ...
    private void displayLimitedDicomInfo(DicomSpecialElement media) {
        if (media != null) {
            RTReader reader = new RTReader(this.rtSeries, media);

            //TODO: here according to series type I can access necessary tool from view and put loaded data from Dicom into it
            SampleTool st = (SampleTool) RTContainer.TOOLS.get(4);
            st.setModel(reader.getStructures());
        }
    }
    ...
    private void updatePrButtonState(DicomImageElement img, boolean newImg) {
        if (img == null || newImg) {
            ...
            for (int i = getViewButtons().size() - 1; i >= 0; i--) {
                ...
                if (vb != null && (vb.getIcon() == RTView.PR_ICON)) {
    ...
    protected void addCrossline(DicomImageElement selImage, DicomImageElement curImage, LocalizerPoster localizer, boolean center) {
        ...
        public boolean importData(TransferSupport support) {
            ...
            try {
                ...
                if (seq instanceof DicomSeries && model instanceof TreeModel) {
                    ...
                    if (selPlugin instanceof RTContainer
                            && ((RTContainer) selPlugin).isContainingView(RTView.this)
                        ...
                        synchronized (UIManager.VIEWER_PLUGINS) {
                            plugin: for (final ViewerPlugin<?> p : UIManager.VIEWER_PLUGINS) {
                                if (p1.equals(p.getGroupID())) {
                                    if (!((RTContainer) p).isContainingView(RTView.this)) {
                                        openPlugin = p;
                                    }
                                    break plugin;
                                }
            ...
            if (selPlugin != null && selPlugin.isContainingView(RTView.this)) {
                selPlugin.setSelectedImagePaneFromFocus(RTView.this);
            }
            if (selList != null) {
                selList.setOpenningSeries(false);
            }
        ...
        if (selected != null) {
                    ...
                    if (absgraph.isVariablePointsNumber()) {
                        if (graphicComplete) {
                            ...
                            final MouseEventDouble mouseEvt =
                                    new MouseEventDouble(RTView.this, MouseEvent.MOUSE_RELEASED, evt.getWhen(), 16, 0, 0,
                                            0, 0, 1, true, 1);
                            mouseEvt.setSource(RTView.this);
                                ...
                                menuItem.addActionListener(new ActionListener() {
                                    ...
                                    public void actionPerformed(ActionEvent e) {
                                        ...
                                        MouseEventDouble evt2 =
                                                new MouseEventDouble(RTView.this, MouseEvent.MOUSE_PRESSED, evt.getWhen(),
                            ...
                            item2.addActionListener(new ActionListener() {
                                ...
                                public void actionPerformed(ActionEvent e) {
                                    MouseEventDouble event =
                                            new MouseEventDouble(RTView.this, 0, 0, 16, 0, 0, 0, 0, 2, true, 1);
            ...
            if (graphicComplete) {
                ...
                menuItem.addActionListener(new ActionListener() {
                    ...
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        RTView.this.getLayerModel().deleteSelectedGraphics(true);
                    }
                });
                popupMenu.add(menuItem);
                ...
                menuItem.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        RTView.this.getLayerModel().deleteSelectedGraphics(false);
                    }
                });
                popupMenu.add(menuItem);
                ...
                if (graphicComplete && graph instanceof LineGraphic) {
                    ...
                    calibMenu.addActionListener(new ActionListener() {
                        ...
                        public void actionPerformed(ActionEvent e) {
                            ...
                            CalibrationView calibrationDialog = new CalibrationView((LineGraphic) graph, RTView.this);
                            ColorLayerUI layer = ColorLayerUI.createTransparentLayerUI(RTView.this);
                            int res =
            ...
            if (list.size() > 0) {
                ...
                properties.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ColorLayerUI layer = ColorLayerUI.createTransparentLayerUI(RTView.this);
                        JDialog dialog = new MeasureDialog(RTView.this, list);
        ...
        if (AbstractLayerModel.GraphicClipboard.getGraphics() != null) {
            ...
            menuItem.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (graphs != null) {
                        Rectangle2D area = RTView.this.getViewModel().getModelArea();
                        for (Graphic g : graphs) {
                            if (!g.getBounds(null).intersects(area)) {
                                int option =
                                        JOptionPane.showConfirmDialog(RTView.this,
                        ...
                        for (Graphic g : graphs) {
                            AbstractLayer layer = RTView.this.getLayerModel().getLayer(g.getLayerID());
                            if (layer != null) {
                                Graphic graph = g.deepCopy();
                                if (graph != null) {
                                    graph.updateLabel(true, RTView.this);
                                    layer.addGraphic(graph);
                                }
                            }
                        }
                        // Repaint all because labels are not drawn
                        RTView.this.getLayerModel().repaint();
                    }
                }
            });
            popupMenu.add(menuItem);
        ...
        if (BundleTools.SYSTEM_PREFERENCES.getBooleanProperty("weasis.contextmenu.close", true)) { //$NON-NLS-1$
            ...
            close.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    RTView.this.setSeries(null, null);
                }
            });
            popupMenu.add(close);
        }
        ...
        private void showPopup(final MouseEvent evt) {
            ...
            if ((evt.getModifiersEx() & getButtonMaskEx()) != 0) {
                ...
                final ArrayList<Graphic> selected =
                        new ArrayList<Graphic>(RTView.this.getLayerModel().getSelectedGraphics());
                if (selected.size() > 0) {
                    popupMenu = RTView.this.buildGraphicContextMenu(evt, selected);
                } else if (RTView.this.getSourceImage() != null) {
                    popupMenu = RTView.this.buildContexMenu(evt);
                }
                if (popupMenu != null) {
