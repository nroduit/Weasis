...
import org.weasis.dicom.rt.OpenDicomAction;

@Component(immediate = false)
@Service
@Property(name = "service.name", value = "RT DICOM Viewer")
public class RTFactory implements SeriesViewerFactory {

    public static final String NAME = Messages.getString("RTFactory.viewer"); //$NON-NLS-1$
    ...
    public SeriesViewer<? extends MediaElement<?>> createSeriesViewer(Map<String, Object> properties) {
        ...
        RTContainer instance = new RTContainer(model, uid, getUIName(), getIcon(), null);
        if (properties != null) {
    ...
    public static void closeSeriesViewer(RTContainer view2dContainer) {
    ...
    public boolean canReadMimeType(String mimeType) {
        return  RTElementFactory.SERIES_RT_MIMETYPE.equals(mimeType);
    }

    @Override
    public boolean isViewerCreatedByThisFactory(SeriesViewer<? extends MediaElement<?>> viewer) {
        if (viewer instanceof RTContainer) {
            return true;
        }
        return false;
    }
