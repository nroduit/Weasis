...
import org.weasis.dicom.rt.dockable.DisplayTool;
import org.weasis.dicom.rt.dockable.ImageTool;

public class RTContainer extends ImageViewerPlugin<DicomImageElement> implements PropertyChangeListener {

    private static final Logger LOGGER = LoggerFactory.getLogger(RTContainer.class);
    ...
    public static final GridBagLayoutModel VIEWS_2x1_r1xc2_dump =
            new GridBagLayoutModel(
                    RTContainer.class.getResourceAsStream("/config/layoutModel.xml"), "layout_dump", Messages.getString("View2dContainer.layout_dump"), new ImageIcon( //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    RTContainer.class.getResource("/icon/22x22/layout1x2_c2.png"))); //$NON-NLS-1$
    ...
    public RTContainer() {
        this(VIEWS_1x1, null, RTFactory.NAME, RTFactory.ICON, null);
    }

    public RTContainer(GridBagLayoutModel layoutModel, String uid, String pluginName, Icon icon, String tooltips) {
...
//            }
            ...
            if (InsertableUtil.getBooleanProperty(BundleTools.SYSTEM_PREFERENCES, bundleName, componentName,
                    InsertableUtil.getCName(SampleTool.class), key, true)) {
                tool = new SampleTool(eventManager);
                TOOLS.add(tool);
            }
...
//                    menu.add(mip);
    ...
    public void close() {
        super.close();
        RTFactory.closeSeriesViewer(this);
    ...
    private void setKOSpecialElement(KOSpecialElement updatedKOSelection, Boolean enableFilter, boolean forceUpdate,
        ...
        if (updatedKOSelection != null && selectedView instanceof RTView) {
            if (SynchData.Mode.Tile.equals(this.getSynchView().getSynchData().getMode())) {
                ...
                if (updateAll) {
                    ...
                    for (DefaultView2d<DicomImageElement> view : viewList) {
                        ((RTView) view).updateKOButtonVisibleState();
                    }
                } else {
                    ((RTView) selectedView).updateKOButtonVisibleState();
                }
                ...
                for (DefaultView2d<DicomImageElement> view : viewList) {
                    ...
                    if ((view.getSeries() instanceof DicomSeries) == false || (view instanceof RTView) == false) {
                        continue;
                    }
    ...
    public DefaultView2d<DicomImageElement> createDefaultView(String classType) {
        return new RTView(eventManager);
    }
    ...
    public List<Action> getExportActions() {
        ...
        if (AppProperties.OPERATING_SYSTEM.startsWith("mac")) { //$NON-NLS-1$
            AbstractAction importAll =
                    new AbstractAction(
                            Messages.getString("View2dContainer.expOsirixMes"), new ImageIcon(RTContainer.class.getResource("/icon/16x16/osririx.png"))) { //$NON-NLS-1$//$NON-NLS-2$
                        ...
                        public void actionPerformed(ActionEvent e) {
                            ...
                            try {
                                ...
                                if (val != 0) {
                                    JOptionPane.showMessageDialog(RTContainer.this,
    ...
    public List<Action> getPrintActions() {
        ...
        AbstractAction printStd =
                new AbstractAction(title, new ImageIcon(ImageViewerPlugin.class.getResource("/icon/16x16/printer.png"))) { //$NON-NLS-1$
                    ...
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        ColorLayerUI layer = ColorLayerUI.createTransparentLayerUI(RTContainer.this);
                        PrintDialog dialog =
                                new PrintDialog(SwingUtilities.getWindowAncestor(RTContainer.this), title, eventManager);
        ...
        AbstractAction printStd2 = new AbstractAction(title2, null) {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                DicomPrintDialog dialog =
                        new DicomPrintDialog(SwingUtilities.getWindowAncestor(RTContainer.this), title2, eventManager);
                ColorLayerUI.showCenterScreen(dialog, layer);
            }
        };
