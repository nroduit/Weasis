...
public class RTReader {
    ...
    public RTReader(Series series, DicomSpecialElement dicomRT) {
        ...
        this.dicomRT = dicomRT;
        if (dicomRT.getMediaReader() instanceof DicomMediaIO) {
...
//                        random.randint(0,255)), dtype=float)
                ...
                Map<Float, ArrayList<Contour>> planes = new HashMap<Float, ArrayList<Contour>>();
        ...
        if (dcmItems != null && dcmItems.getString(Tag.SOPClassUID).equals("1.2.840.10008.5.1.4.1.1.481.5")) {
            ...
            if (plan.getRxDose() == 0.0f) {
                for (Attributes fractionGroup : dcmItems.getSequence(Tag.FractionGroupSequence)) {
                    ...
                    if (fractionGroup.contains(Tag.ReferencedBeamSequence) && fractionGroup.contains(Tag.NumberOfFractionsPlanned)) {
...
//        return beams
     ...
     * @param array array of strings representing x,y,z coordinates
     * @return array of 3d point maps
     */
    private List<Map<Character, Float>> getContourPoints(String[] array) {
        List<Map<Character, Float>> points3d = new ArrayList<Map<Character, Float>>();

        for (int i = 0; i < array.length; i += 3) {
            Map<Character, Float> point3d = new HashMap<Character, Float>();
     ...
     * @param array array of strings representing r,g,b parts
     * @return color map
     */
    private Map<Character, Float> getColor(String[] array) {
        Map<Character, Float> color = new HashMap<Character, Float>();
