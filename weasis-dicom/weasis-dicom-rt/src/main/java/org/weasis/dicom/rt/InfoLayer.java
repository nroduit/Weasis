...
import org.weasis.dicom.codec.display.*;
...
public class InfoLayer implements AnnotationsLayer {
    ...
    public void paint(Graphics2D g2) {
        ...
        if (!image.isReadable()) {
            String message = org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.msg_not_read"); //$NON-NLS-1$
            float y = midy;
            ...
            if (StringUtil.hasText(tsuid)) {
                tsuid = org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.tsuid") + StringUtil.COLON_AND_SPACE + tsuid; //$NON-NLS-1$
                y += fontHeight;
        ...
        if (dcm != null) {
            ...
            if ("01".equals(dcm.getTagValue(TagW.LossyImageCompression))) { //$NON-NLS-1$
                ...
                StringBuilder buf =
                    new StringBuilder(org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.lossy") + StringUtil.COLON_AND_SPACE);//$NON-NLS-1$
        ...
        if (getDisplayPreferences(PIXEL)) {
            StringBuilder sb = new StringBuilder(org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.pixel")); //$NON-NLS-1$
        ...
        if (getDisplayPreferences(WINDOW_LEVEL)) {
            ...
            if (window != null && level != null) {
                sb.append(org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.wl"));//$NON-NLS-1$
                sb.append(StringUtil.COLON_AND_SPACE);
                ...
                if (dcm != null) {
                    ...
                    if (minp > maxModLUT || maxp < minModLUT) {
                        outside = true;
                        sb.append(" - "); //$NON-NLS-1$
                        sb.append(org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.msg_outside_levels")); //$NON-NLS-1$
                    }
                }
            }
            if (outside) {
        ...
        if (getDisplayPreferences(ZOOM)) {
            GraphicLabel
                .paintFontOutline(
                    g2,
                    org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.zoom") + StringUtil.COLON_AND_SPACE + DecFormater.percentTwoDecimal(view2DPane.getViewModel().getViewScale()) //$NON-NLS-1$
                    , border, drawY);
            drawY -= fontHeight;
        }
        if (getDisplayPreferences(ROTATION)) {
            GraphicLabel
                .paintFontOutline(
                    g2,
                    org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.angle") + StringUtil.COLON_AND_SPACE + disOp.getParamValue(RotationOp.OP_NAME, RotationOp.P_ROTATE) + " " + org.weasis.dicom.viewer2d.Messages.getString("InfoLayer.angle_symb"), border, drawY); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            drawY -= fontHeight;
        }
