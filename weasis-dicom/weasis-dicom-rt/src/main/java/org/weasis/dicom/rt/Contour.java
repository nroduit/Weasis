  ...
  public Graphic getGraphic(GeometryOfSlice geometry) {
    if (geometry != null && points != null && points.length % 3 == 0) {
      ...
      for (int i = 1; i < size; i++) {
        x =
            ((points[i * 3] - tlhc.x) * row.x
                    + (points[i * 3 + 1] - tlhc.y) * row.y
                    + (points[i * 3 + 2] - tlhc.z) * row.z)
                / voxelSpacing.x;
        y =
            ((points[i * 3] - tlhc.x) * column.x
                    + (points[i * 3 + 1] - tlhc.y) * column.y
                    + (points[i * 3 + 2] - tlhc.z) * column.z)
                / voxelSpacing.y;
        path.lineTo(x, y);
      }
  ...
  public boolean containsContour(Contour contour) {
    ...
    for (int i = 0; i < this.getContourPoints(); i++) {
      if (this.getCoordinatesY()[i] < contour.getCoordinateY()
        ...
        if (this.getCoordinatesX()[i]
                + (contour.getCoordinateY() - this.getCoordinatesY()[i])
                    / (this.getCoordinatesY()[j] - this.getCoordinatesY()[i])
                    * (this.getCoordinatesX()[j] - this.getCoordinatesX()[i])
            < contour.getCoordinateX()) {
          isInside = !isInside;
        }
      }
