...
public class DicomModelQueryResult extends AbstractQueryResult {
  ...
  private void init(DicomModel model, MediaSeriesGroup... patientGroups) {
    int imgNotAdd = 0;
    Collection<MediaSeriesGroup> pts =
        patientGroups == null || patientGroups.length == 0
            ? model.getChildren(MediaSeriesGroupNode.rootNode)
            : Arrays.asList(patientGroups);
      ...
      for (MediaSeriesGroup study : model.getChildren(patient)) {
        ...
        for (MediaSeriesGroup series : model.getChildren(study)) {
          ...
          if (series instanceof MediaSeries) {
            ...
            if (wadoParams == null
                || !StringUtil.hasText(wadoParams.getBaseURL())
            ...
            if (DicomModel.isSpecialModality((MediaSeries<?>) series)) {
              ...
              for (MediaElement media :
                  ((MediaSeries<MediaElement>) series).getSortedMedias(null)) {
  ...
  public static Study getStudy(MediaSeriesGroup study, Patient patient) {
    String uid =
        TagD.getTagValue(Objects.requireNonNull(study), Tag.StudyInstanceUID, String.class);
    ...
    if (s == null) {
      ...
      s.setReferringPhysicianName(
          TagD.getTagValue(study, Tag.ReferringPhysicianName, String.class));
      patient.addStudy(s);
    }
    return s;
  }
  ...
  public static Series getSeries(MediaSeriesGroup series, Study study) {
    String uid =
        TagD.getTagValue(Objects.requireNonNull(series), Tag.SeriesInstanceUID, String.class);
    ...
    if (s == null) {
      ...
      s.setSeriesNumber(
          StringUtil.getNullIfNull(TagD.getTagValue(series, Tag.SeriesNumber, Integer.class)));
      ...
      s.setWadoTransferSyntaxUID(
          TagW.getTagValue(series, TagW.WadoTransferSyntaxUID, String.class));
