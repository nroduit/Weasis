...
public class ShadingPrefDialog extends JDialog {
  ...
  private void init() {
    ...
    powerSlider = createSlider("Power", 1, 50, realToSliderPowerValue(specularPower));
    contentPane.add(GuiUtils.boxVerticalStrut(Insertable.BLOCK_SEPARATOR));
    ...
    restoreButton.addActionListener(
        e -> {
          ...
          updateSliderText();
          render = true;
          updateValues(p.getAmbient(), p.getDiffuse(), p.getSpecular(), p.getSpecularPower());
        });
    ...
    cancelButton.addActionListener(
        e -> {
          updateValues(ambient, diffuse, specular, specularPower);
          dispose();
        });
  ...
  private void updateSliderText() {
    ...
    powerSlider.setValue(realToSliderPowerValue(specularPower));
    result = "Power" + StringUtil.COLON_AND_SPACE + DecFormatter.oneDecimal(specularPower);
    SliderChangeListener.updateSliderProperties(powerSlider, result);
  }

  private void updateValues(float ambient, float diffuse, float specular, float specularPower) {
    ShadingOptions options = view3d.getRenderingLayer().getShadingOptions();
