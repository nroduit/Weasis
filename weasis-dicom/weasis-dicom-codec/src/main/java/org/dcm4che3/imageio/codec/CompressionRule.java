...
public class CompressionRule implements Comparable<CompressionRule>, Serializable {
    ...
    public CompressionRule(String commonName, String[] pmis, int[] bitsStored, int pixelRepresentation,
        ...
        this.condition = new Condition(pmis, bitsStored, pixelRepresentation, StringUtils.maskNull(aeTitles),
            StringUtils.maskNull(sopClasses), StringUtils.maskNull(bodyPartExamined));
        this.tsuid = tsuid;
    ...
    public boolean matchesCondition(PhotometricInterpretation pmi, int bitsStored, int pixelRepresentation,
        String aeTitle, String sopClass, String bodyPart) {
        return condition.matches(pmi, bitsStored, pixelRepresentation, aeTitle, sopClass, bodyPart);
    }
    ...
    private static class Condition implements Comparable<Condition>, Serializable {
        ...
        public boolean matches(PhotometricInterpretation pmi, int bitsStored, int pixelRepresentation, String aeTitle,
            String sopClass, String bodyPart) {
            return pmis.contains(pmi) && matchBitStored(bitsStored) && matchPixelRepresentation(pixelRepresentation)
                && isEmptyOrContains(this.aeTitles, aeTitle) && isEmptyOrContains(this.sopClasses, sopClass)
