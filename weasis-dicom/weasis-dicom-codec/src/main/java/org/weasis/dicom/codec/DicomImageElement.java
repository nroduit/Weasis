...
public class DicomImageElement extends ImageElement {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(DicomImageElement.class);

	private static final SoftHashMap<LutParameters, LookupTableJAI> LUT_Cache = new SoftHashMap<LutParameters, LookupTableJAI>() {

		public Reference<? extends LookupTableJAI> getReference(
				LutParameters key) {
			return hash.get(key);
		}
	...
	public void initPixelConfiguration() {
		...
		if (!"SC".equals(modality) && !"OT".equals(modality)) { //$NON-NLS-1$ //$NON-NLS-2$
			// Physical distance in mm between the center of each pixel (ratio
			// in mm)
			val = TagD.getTagValue(mediaIO, Tag.PixelSpacing, double[].class);
			if (val == null || val.length != 2) {
				val = TagD.getTagValue(mediaIO, Tag.ImagerPixelSpacing,
						double[].class);
				// Follows D. Clunie recommendations
				pixelSizeCalibrationDescription = val == null ? null
						: Messages.getString("DicomImageElement.detector"); //$NON-NLS-1$

			} else {
				pixelSizeCalibrationDescription = TagD.getTagValue(mediaIO,
						Tag.PixelSpacingCalibrationDescription, String.class);
			}
			if (val != null && val.length == 2 && val[0] > 0.0
					&& val[1] > 0.0) {
				/*
				 * Pixel Spacing = Row Spacing \ Column Spacing => (Y,X) The
				 * first value is the row spacing in mm, that is the spacing
				 * between the centers of adjacent rows, or vertical spacing.
				 * Pixel Spacing must be always positive, but some DICOMs have
				 * negative values
				 */
			...
			// Specifies the units of the output of the Modality LUT or rescale
			// operation.
			// Defined Terms:
			// OD = The number in the LUT represents thousands of optical
			// density. That is, a value of
			// 2140 represents an optical density of 2.140.
			// HU = Hounsfield Units (CT)
			// US = Unspecified
			// Other values are permitted, but are not defined by the DICOM
			// Standard.
			pixelValueUnit = TagD.getTagValue(this, Tag.RescaleType,
					String.class);
			if (pixelValueUnit == null) {
				// For some other modalities like PET
				pixelValueUnit = TagD.getTagValue(this, Tag.Units,
						String.class);
			}
		...
		if (val == null) {
			int[] aspects = TagD.getTagValue(mediaIO, Tag.PixelAspectRatio,
					int[].class);
			if (aspects != null && aspects.length == 2
					&& aspects[0] != aspects[1]) {
				/*
				 * Set the Pixel Aspect Ratio to the pixel size of the image to
				 * stretch the rendered image (for having square pixel on the
				 * display image)
				 */
				if (aspects[1] < aspects[0]) {
					setPixelSize(1.0,
							(double) aspects[0] / (double) aspects[1]);
				} else {
					setPixelSize((double) aspects[1] / (double) aspects[0],
							1.0);
				}
			}
		}
	...
	/**
	 * @return return the min value after modality pixel transformation and
	 *         after pixel padding operation if padding exists.
	 */
	@Override
	public double getMinValue(TagReadable tagable, boolean pixelPadding) {
		// Computes min and max as slope can be negative
		return Math.min(
				pixel2mLUT(super.getMinValue(tagable, pixelPadding), tagable,
						pixelPadding),
				pixel2mLUT(super.getMaxValue(tagable, pixelPadding), tagable,
						pixelPadding));
	}

	/**
	 * @return return the max value after modality pixel transformation and
	 *         after pixel padding operation if padding exists.
	 */
	@Override
	 ...
	 * pixel representation. Enumerated Values: 0000H = unsigned integer. 0001H
	 * = 2's complement
	 *
	...
	public boolean isPixelRepresentationSigned() {
		Integer pixelRepresentation = TagD.getTagValue(this,
				Tag.PixelRepresentation, Integer.class);
	...
	public boolean isPhotometricInterpretationInverse(TagReadable tagable) {
		String prLUTShape = TagD.getTagValue(tagable, Tag.PresentationLUTShape,
				String.class);
		if (prLUTShape == null) {
			prLUTShape = TagD.getTagValue(this, Tag.PresentationLUTShape,
					String.class);
		}
	...
	/**
	 * In the case where Rescale Slope and Rescale Intercept are used for
	 * modality pixel transformation, the output ranges may be signed even if
	 ...
	 * @return
	 */
	public boolean isModalityLutOutSigned(TagReadable tagable,
	...
	public double getRescaleIntercept(TagReadable tagable) {
		Double prIntercept = TagD.getTagValue(tagable, Tag.RescaleIntercept,
				Double.class);
		Double intercept = prIntercept == null
	...
	public double getRescaleSlope(TagReadable tagable) {
		Double prSlope = TagD.getTagValue(tagable, Tag.RescaleSlope,
				Double.class);
		Double slope = prSlope == null
				? TagD.getTagValue(this, Tag.RescaleSlope, Double.class)
				: prSlope;
		return slope == null ? 1.0 : slope;
	}

	public int pixel2mLUT(Number pixelValue, TagReadable tagable,
			boolean pixelPadding) {
		if (pixelValue != null) {
			int val = pixelValue.intValue();
			LookupTableJAI lookup = getModalityLookup(tagable, pixelPadding);
			if (lookup != null) {
				if (val >= lookup.getOffset()
						&& val < lookup.getOffset() + lookup.getNumEntries()) {
	...
	public int getMinAllocatedValue(TagReadable tagable, boolean pixelPadding) {
		...
		int maxValue = signed ? (1 << (bitsAllocated - 1)) - 1
				: ((1 << bitsAllocated) - 1);
	...
	public int getMaxAllocatedValue(TagReadable tagable, boolean pixelPadding) {
		boolean signed = isModalityLutOutSigned(tagable, pixelPadding);
		int bitsAllocated = getBitsAllocated();
		return signed ? (1 << (bitsAllocated - 1)) - 1
				: ((1 << bitsAllocated) - 1);
	}
	...
	/**
	 * The value of Photometric Interpretation specifies the intended
	 * interpretation of the image pixel data.
	 *
	 * @return following values (MONOCHROME1 , MONOCHROME2 , PALETTE COLOR ....)
	 *         Other values are permitted but the meaning is not defined by this
	 *         Standard.
	 */
	public String getPhotometricInterpretation() {
		return TagD.getTagValue(this, Tag.PhotometricInterpretation,
				String.class);
	}
	...
	/**
	 *
	 * Pixel Padding Value is used to pad grayscale images (those with a
	 * Photometric Interpretation of MONOCHROME1 or MONOCHROME2)<br>
	 * Pixel Padding Value specifies either a single value of this padding
	 * value, or when combined with Pixel Padding Range Limit, a range of values
	 * (inclusive) that are padding.<br>
	 * <br>
	 * <b>Note :</b> It is explicitly described in order to prevent display
	 * applications from taking it into account when determining the dynamic
	 * range of an image, since the Pixel Padding Value will be outside the
	 * range between the minimum and maximum values of the pixels in the native
	 * image
	 *
	 * @see DICOM standard PS 3.3 - §C.7.5.1.1.2 - Pixel Padding Value and Pixel
	 *      Padding Range Limit
	 */
	...
	public Integer getPaddingLimit() {
		return TagD.getTagValue(this, Tag.PixelPaddingRangeLimit,
				Integer.class);

	}

	public LutParameters getLutParameters(TagReadable tagable,
			boolean pixelPadding, LookupTableJAI mLUTSeq,
		...
		if (bitsStored > 16 || (MathUtil.isEqual(slope, 1.0)
				&& MathUtil.isEqualToZero(intercept) && paddingValue == null)) {
			return null;
		}
		...
		int bitsOutputLut;
		if (mLUTSeq == null) {
			double minValue = super.getMinValue(tagable, pixelPadding) * slope
					+ intercept;
			double maxValue = super.getMaxValue(tagable, pixelPadding) * slope
					+ intercept;
			bitsOutputLut = Integer.SIZE - Integer.numberOfLeadingZeros(
					(int) Math.round(maxValue - minValue));
			...
			bitsOutputLut = mLUTSeq.getDataType() == DataBuffer.TYPE_BYTE ? 8
					: 16;
		}
		return new LutParameters(intercept, slope, pixelPadding, paddingValue,
				paddingLimit, bitsStored, isSigned, outputSigned, bitsOutputLut,
				inversePaddingMLUT);

	}

	public LookupTableJAI getModalityLookup(TagReadable tagable,
			boolean pixelPadding) {
	 ...
	 * Modality LUT Sequence (0028,3000) are used. In the case where Rescale
	 * Slope and Rescale Intercept are used, the output ranges from (minimum
	 * pixel value*Rescale Slope+Rescale Intercept) to (maximum pixel
	 ...
	 * from 0 to 2n-1 where n is the third value of LUT Descriptor. This range
	 * is always unsigned.
	 *
	 * @param pixelPadding
	 * @param inverseLUT
	 ...
	 */
	protected LookupTableJAI getModalityLookup(TagReadable tagable,
			boolean pixelPadding, boolean inverseLUTAction) {
		Integer paddingValue = getPaddingValue();
		LookupTableJAI prModLut = (LookupTableJAI) (tagable != null
				? tagable.getTagValue(TagW.ModalityLUTData) : null);
		final LookupTableJAI mLUTSeq = prModLut == null
				? (LookupTableJAI) getTagValue(TagW.ModalityLUTData) : prModLut;
		if (mLUTSeq != null) {
			if (!pixelPadding || paddingValue == null) {
				if (super.getMinValue(tagable, false) >= mLUTSeq.getOffset()
						&& super.getMaxValue(tagable,
								false) < mLUTSeq.getOffset()
										+ mLUTSeq.getNumEntries()) {
					return mLUTSeq;
				...
				LOGGER.warn(
						"Cannot apply Modality LUT sequence and Pixel Padding"); //$NON-NLS-1$
			}
		}
		...
		LutParameters lutparams = getLutParameters(tagable, pixelPadding,
				mLUTSeq, inverseLut);
		...
		if (mLUTSeq != null) {
			if (mLUTSeq.getNumBands() == 1) {
				if (mLUTSeq.getDataType() == DataBuffer.TYPE_BYTE) {
					...
					if (data != null) {
						modalityLookup = new LookupTableJAI(data,
								mLUTSeq.getOffset(0));
					}
				} else {
		...
		if (isPhotometricInterpretationMonochrome()) {
			DicomImageUtils.applyPixelPaddingToModalityLUT(modalityLookup,
					lutparams);
		}
	 ...
	 */
	public LookupTableJAI getVOILookup(TagReadable tagable, Double window,
			Double level, Double minLevel, Double maxLevel, LutShape shape,
			boolean fillLutOutside, boolean pixelPadding) {

		if (window == null || level == null || shape == null || minLevel == null
				|| maxLevel == null) {
			return null;
		}

		if (getPaddingValue() != null
				&& isPhotometricInterpretationMonochrome()) {
			/*
			 * When pixel padding is activated, VOI LUT must extend to the min
			 * bit stored value when MONOCHROME2 and to the max bit stored value
			 * when MONOCHROME1.
			 *
			 * C.7.5.1.1.2 Pixel Padding Value and Pixel Padding Range Limit If
			 * Photometric Interpretation
			 *
			 * (0028,0004) is MONOCHROME2, Pixel Padding Value (0028,0120) shall
			 * be less than (closer to or equal to the minimum possible pixel
			 * value) or equal to Pixel Padding Range Limit (0028,0121). If
			 * Photometric Interpretation (0028,0004) is MONOCHROME1, Pixel
			 * Padding Value (0028,0120) shall be greater than (closer to or
			 * equal to the maximum possible pixel value) or equal to Pixel
			 * Padding Range Limit (0028,0121).
			 */

			// Create a VOI LUT with pixel padding values at the extremity of
		...
		return DicomImageUtils.createWindowLevelLut(shape, window, level,
				minValue, maxValue, 8, false,
				isPhotometricInterpretationInverse(tagable));
	}

	/**
	 * @return default as first element of preset List <br>
	 *         Note : null should never be returned since auto is at least one
	 *         preset
	 */
	public PresetWindowLevel getDefaultPreset(boolean pixelPadding) {
		...
		return (presetList != null && !presetList.isEmpty()) ? presetList.get(0)
				: null;
	}

	public List<PresetWindowLevel> getPresetList(boolean pixelPadding) {
		if (windowingPresetCollection == null && isImageAvailable()) {
			windowingPresetCollection = PresetWindowLevel
					.getPresetCollection(this, this, pixelPadding);
		}
	...
	public Histogram getHistogram(RenderedImage imageSource,
			TagReadable tagable, boolean pixelPadding) {
		LookupTableJAI lookup = getModalityLookup(tagable, pixelPadding);
		if (imageSource == null || lookup == null) {
			return null;
		}
		// TODO instead of computing histo from image get Dicom attribute if
		// present. Handle pixel padding!

		ParameterBlock pb = new ParameterBlock();
		pb.addSource(imageSource);
		pb.add(lookup);
		final RenderedImage imageModalityTransformed = JAI.create("lookup", pb, //$NON-NLS-1$
				null);
		...
		pb.add(new double[] { getMinAllocatedValue(tagable, pixelPadding) }); // Min
																				// value
																				// to
																				// be
																				// considered.
		pb.add(new double[] { getMaxAllocatedValue(tagable, pixelPadding) }); // Max
																				// value
																				// to
																				// be
																				// considered.
	...
	protected void findMinMaxValues(RenderedImage img,
			boolean exclude8bitImage) {
		/*
		 * This function can be called several times from the inner class Load.
		 * min and max will be computed only once.
		 */

		if (img != null && !isImageAvailable()) {
			// Cannot trust SmallestImagePixelValue and LargestImagePixelValue
			// values! So search min and max values
			int bitsStored = getBitsStored();
			int bitsAllocated = getBitsAllocated();
			...
			if (monochrome) {
				...
				if (paddingValue != null) {
					Integer paddingLimit = getPaddingLimit();
					int paddingValueMin = (paddingLimit == null) ? paddingValue
							: Math.min(paddingValue, paddingLimit);
					int paddingValueMax = (paddingLimit == null) ? paddingValue
							: Math.max(paddingValue, paddingLimit);
			...
			if (bitsStored < bitsAllocated && isImageAvailable()) {
				...
				int maxInValue = isSigned ? (1 << (bitsStored - 1)) - 1
						: (1 << bitsStored) - 1;
				if (minPixelValue < minInValue || maxPixelValue > maxInValue) {
					/*
					 *
					 *
					 * When the image contains values outside the bits stored
					 * values, the bits stored is replaced by the bits allocated
					 * for having a LUT which handles all the values.
					 *
					 * Overlays in pixel data should be masked before finding
					 * min and max.
					 */
			...
			/*
			 * Lazily compute image pixel transformation here since inner class
			 * Load is called from a separate and dedicated worker Thread. Also,
			 * it will be computed only once
			 *
			 * Considering that the default pixel padding option is true and
			 * Inverse LUT action is false
			 */
	 ...
	 */
	private void findMinMaxValues(RenderedImage img, double paddingValueMin,
			double paddingValueMax) {
		if (img != null) {
			...
			if (datatype == DataBuffer.TYPE_BYTE) {
				...
				RenderedOp dst = ImageStatisticsDescriptor.create(img,
						(ROI) null, 1, 1, new Double(paddingValueMin),
				...
				// Handle special case when min and max are equal, ex. black
				// image
	...
	public double getFullDynamicWidth(TagReadable tagable,
			boolean pixelPadding) {
		return getMaxValue(tagable, pixelPadding)
				- getMinValue(tagable, pixelPadding);
	}

	public double getFullDynamicCenter(TagReadable tagable,
			boolean pixelPadding) {
	...
	public LutShape getDefaultShape(boolean pixelPadding) {
		...
		return (defaultPreset != null) ? defaultPreset.getLutShape()
	...
	public double getDefaultLevel(boolean pixelPadding) {
		PresetWindowLevel defaultPreset = getDefaultPreset(pixelPadding);
		return (defaultPreset != null) ? defaultPreset.getLevel()
				: super.getDefaultLevel(pixelPadding);
	 ...
	 * @param window
	 *            is width from low to high input values around level. If null,
	 *            getDefaultWindow() value is used
	 * @param level
	 *            is center of window values. If null, getDefaultLevel() value
	 *            is used
	 * @param lutShape
	 *            defines the shape of applied lookup table transformation. If
	 *            null getDefaultLutShape() is used
	 * @param pixPadding
	 *            indicates if some padding values defined in ImageElement
	 *            should be applied or not. If null, TRUE is considered
	 * @return
	 */
	@Override
	public RenderedImage getRenderedImage(final RenderedImage imageSource,
		...
		if (params != null) {
			...
			pixelPadding = (Boolean) params
					.get(ActionW.IMAGE_PIX_PADDING.cmd());
			...
			if (prTags != null) {
				prLutData = (LookupTableJAI) prTags
						.getTagValue(TagW.PRLUTsData);
			}
		...
		double windowValue = (window == null) ? getDefaultWindow(pixPadding)
				: window;
		double levelValue = (level == null) ? getDefaultLevel(pixPadding)
				: level;
		LutShape lut = (lutShape == null) ? getDefaultShape(pixPadding)
				: lutShape;
		double minLevel;
		double maxLevel;
		if (levelMin == null || levelMax == null) {
			minLevel = Math.min(levelValue - windowValue / 2.0,
					getMinValue(prTags, pixPadding));
			maxLevel = Math.max(levelValue + windowValue / 2.0,
					getMaxValue(prTags, pixPadding));
		} else {
		...
		if (datatype >= DataBuffer.TYPE_BYTE
			...
			LookupTableJAI modalityLookup = getModalityLookup(prTags,
					pixPadding, invLUT);

			// RenderingHints hints = new RenderingHints(JAI.KEY_IMAGE_LAYOUT,
			// new ImageLayout(imageSource));
			RenderedImage imageModalityTransformed = modalityLookup == null
					? imageSource
					: LookupDescriptor.create(imageSource, modalityLookup,
							null);

			/*
			 * C.11.2.1.2 Window center and window width
			 *
			 * Theses Attributes shall be used only for Images with Photometric
			 * Interpretation (0028,0004) values of MONOCHROME1 and MONOCHROME2.
			 * They have no meaning for other Images.
			 */
			if (!JMVUtils.getNULLtoFalse(wlOnColorImage)
					&& !isPhotometricInterpretationMonochrome()) {
				/*
				 * If photometric interpretation is not monochrome do not apply
				 * VOILUT. It is necessary for PALETTE_COLOR.
				 */
			...
			if (prLutData == null || lut.getLookup() != null) {
				voiLookup = getVOILookup(prTags, windowValue, levelValue,
						minLevel, maxLevel, lut,
						JMVUtils.getNULLtoFalse(fillLutOutside), pixPadding);
			}

			if (prLutData == null) {
				// BUG fix: for some images the color model is null. Creating 8
				// bits gray model layout fixes this issue.
				return LookupDescriptor.create(imageModalityTransformed,
						voiLookup, LayoutUtil.createGrayRenderedImage());
			}

			RenderedImage imageVoiTransformed = voiLookup == null
					? imageModalityTransformed
					: LookupDescriptor.create(imageModalityTransformed,
							voiLookup, null);
			// BUG fix: for some images the color model is null. Creating 8 bits
			// gray model layout fixes this issue.
			return LookupDescriptor.create(imageVoiTransformed, prLutData,
	...
	public GeometryOfSlice getDispSliceGeometry() {
		...
		double[] imgOr = TagD.getTagValue(this, Tag.ImageOrientationPatient,
				double[].class);
		if (imgOr != null && imgOr.length == 6) {
			double[] pos = TagD.getTagValue(this, Tag.ImagePositionPatient,
					double[].class);
			if (pos != null && pos.length == 3) {
				Double sliceTickness = TagD.getTagValue(this,
						Tag.SliceThickness, Double.class);
				if (sliceTickness == null) {
					sliceTickness = getPixelSize();
				}
				double[] spacing = { getPixelSize(), getPixelSize(),
						sliceTickness };
				Integer rows = TagD.getTagValue(this, Tag.Rows, Integer.class);
				Integer columns = TagD.getTagValue(this, Tag.Columns,
						Integer.class);
				if (rows != null && columns != null && rows > 0
						&& columns > 0) {
					...
					return new GeometryOfSlice(
							...
							new double[] { imgOr[3], imgOr[4], imgOr[5] }, pos,
							spacing, sliceTickness,
							new double[] { rows * getRescaleY(),
