...
public class OverlayUtils {
   ...
   * @param params
   */
  public static RenderedImage getBinaryOverlays(
      ImageElement image,
    ...
    IndexColorModel icm =
        new IndexColorModel(
            outBits, icmColorValues.length, icmColorValues, icmColorValues, icmColorValues, 0);
    ...
    if (filePath != null) {
      ...
      try {
        ...
        LOGGER.error("Cannot read serialized overlay", e);
      } finally {
    ...
    for (int i = 0; i < overlayGroupOffsets.length; i++) {
      ...
      if (data != null
          && attributes.getInt(Tag.OverlayBitsAllocated | overlayGroupOffsets[i], 1) != 1
          && data.length > i) {
        ovlyData = data[i];
      }
  ...
  public static RenderedImage getShutterOverlay(
      Attributes attributes, int frameIndex, int width, int height, int shuttOverlayGroup)
      throws IOException {
    IndexColorModel icm =
        new IndexColorModel(
            1, icmColorValues.length, icmColorValues, icmColorValues, icmColorValues, 0);
    BufferedImage overBi = new BufferedImage(width, height, BufferedImage.TYPE_BYTE_BINARY, icm);

    Overlays.applyOverlay(
        frameIndex, overBi.getRaster(), attributes, shuttOverlayGroup - 0x6000, 1, null);
  ...
  public static byte[] extractOverlay(int gg0000, Raster raster, Attributes attrs) {
    ...
    // Binary size = ((imageSize + 7) / 8 ) + 1 & (-2) = 32769 & (-2) = 1000000000000001 &
    // 1111111111111111110
  ...
  public static void extractFromPixeldata(
      Raster raster, int mask, byte[] ovlyData, int off, int length) {
    ...
    switch (db.getDataType()) {
      case DataBuffer.TYPE_BYTE:
        extractFromPixeldata(
            ((DataBufferByte) db).getData(), columns, stride, mask, ovlyData, off, length);
        break;
      case DataBuffer.TYPE_USHORT:
        extractFromPixeldata(
            ((DataBufferUShort) db).getData(), columns, stride, mask, ovlyData, off, length);
        break;
      case DataBuffer.TYPE_SHORT:
        extractFromPixeldata(
            ((DataBufferShort) db).getData(), columns, stride, mask, ovlyData, off, length);
        break;
      default:
  ...
  private static void extractFromPixeldata(
      byte[] pixeldata, int columns, int stride, int mask, byte[] ovlyData, int off, int length) {
  ...
  private static void extractFromPixeldata(
      short[] pixeldata, int columns, int stride, int mask, byte[] ovlyData, int off, int length) {
    for (int y = 0, i = off, imax = off + length; y < columns && i < imax; y++) {
      for (int j = y * stride; j < imax && i < imax; j++, i++) {
        if ((pixeldata[j] & mask) != 0) {
          ovlyData[i >>> 3] |= 1 << (i & 7);
