...
public class CurvedMprImageIO implements DcmMediaReader {
  ...
  private PlanarImage generatePanoramicImage() {
    ...
    List<Vector3d> perpDirs = computePerpendicularDirections(sampledPoints, axis.getPlaneNormal());
    ...
    lastDebugData =
        new DebugCurveData(curvePoints, smoothedPoints, sampledPoints, perpDirs, slabThicknessMm);
   ...
   *
   * <ul>
   *   <li>Tangent: direction along the curve
   *   <li>Normal: perpendicular to tangent, smoothly transported along curve
   *   <li>Binormal: perpendicular to both tangent and normal
   * </ul>
   *
   * <p>The parallel transport frame avoids the twisting that occurs with Frenet-Serret frames at
   * inflection points.
   */
  private void computeParallelTransportFrame(
    ...
    Vector3d firstTangent = tangents.getFirst();
    ...
    for (int idx : sampleIdxs) {
      ...
      LOGGER.info(
          ...
          String.format("%.2f", nn.x),
          String.format("%.2f", nn.y),
          String.format("%.2f", nn.z),
          String.format("%.2f", b.x),
          String.format("%.2f", b.y),
          String.format("%.2f", b.z));
    }
  }
  ...
  /**
   * Compute the perpendicular direction to the curve tangent at each point. The perpendicular is
   * computed in the plane defined by planeNormal (typically XY plane). This direction is used for
   * the slab thickness in MIP.
   *
   * <p>Perpendicular directions are kept consistent along the curve by ensuring each direction
   * doesn't flip relative to its predecessor. This prevents sudden direction changes that would
   * cause sampling artifacts.
   *
   ...
   * polylines into smooth curves.
   *
   * <p>The number of samples per segment is proportional to the segment length to ensure uniform
   * sampling density along the entire curve.
   *
  ...
  /**
   * Resample the curve to have evenly-spaced points. Points are in voxel coordinates. We resample
   * at 1-voxel intervals.
   */
